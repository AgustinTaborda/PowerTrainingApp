...
let CloudfileManagerService = class CloudfileManagerService {
    async uploadImage(file) {
        ...
        return new Promise((resolve, reject) => {
            const upload = cloudinary_1.v2.uploader.upload_stream({ resource_type: 'image', secure: true }, (error, result) => {
                if (error) {
                    reject(error);
                }
                else {
    ...
    async uploadVideo(file) {
        return new Promise((resolve, reject) => {
            const upload = cloudinary_1.v2.uploader.upload_stream({ resource_type: 'video', secure: true }, (error, result) => {
                if (error) {
                    reject(error);
                }
                else {
