...
export class CloudfileManagerController {
  constructor(
    private readonly cloudfileManagerService: CloudfileManagerService,
  ) {}
  ...
  @ApiBody({
    schema: {
      ...
      properties: {
        image: {
          // Asegúrate de que el nombre coincida con el campo del interceptor
          type: 'string',
          format: 'binary',
        },
      },
  ...
  @ApiOperation({
    summary:
      'Retrieve all files stored in the database that you upload to cloudinary',
    description:
      'When you upload a file to cloudinary, it is stored in the database. This endpoint retrieves all files stored in the database.',
  })
  async databaseFiles() {
